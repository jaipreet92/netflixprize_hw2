...
def _predict_rating_using_kdtree(movie_id, user_id, user_to_movies_matrix, movieid_to_idx=data_loader.movieid_to_idx,
                                 userid_to_idx=data_loader.userid_to_idx):
    user_idx = userid_to_idx[user_id]
    movie_idx = movieid_to_idx[movie_id]

    kdtree.build_kdtree_for_user(user_to_movies_matrix, user_id)
    knn_idxs = kdtree.userid_to_knnidxs_map[user_id]

    neighbour_movie_ratings = user_to_movies_matrix[knn_idxs, movie_idx]

    if len(neighbour_movie_ratings) == 0:
        return None
    else:
        return np.mean(neighbour_movie_ratings)
